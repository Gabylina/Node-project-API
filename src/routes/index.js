...
// Placeholder para el middleware de autenticación (Sanctum)
...
// Rutas de autenticación públicas (equivalente a Route::prefix('auth')->group(...))
router.post('/auth/register', AuthController.register);
router.post('/auth/login', AuthController.login);

// Rutas de autenticación protegidas (aplicado solo a rutas que comiencen con /auth)
router.use('/auth', requireAuth);
router.get('/auth/me', AuthController.me);
router.post('/auth/logout', AuthController.logout);

// Rutas que requieren autenticación (equivalente a Route::middleware('auth:sanctum')->group(...))
// Este middleware se aplicará a todas las rutas definidas debajo de este punto.
router.use(requireAuth);

// Rutas para Proyectos (equivalente a apiResource)
