...
const sessionCookieName = process.env.SESSION_COOKIE || appName.replace(/\s+/g, '-').toLowerCase() + '-session';
...
export default {
  ...
  files: process.env.FILESYSTEM_DISK === 'local' ? 'storage/framework/sessions' : 'storage/framework/sessions', // Assuming local disk means local sessions
