...
export default {
  ...
  files: process.env.SESSION_FILES || require('path').resolve(process.cwd(), 'storage', 'framework', 'sessions'),
  connection: process.env.SESSION_CONNECTION,
  table: process.env.SESSION_TABLE || 'sessions',
  store: process.env.SESSION_STORE,
  lottery: [2, 100],
  cookie: process.env.SESSION_COOKIE || appName.replace(/\s+/g, '-').toLowerCase() + '-session',
