...
export default {
  ...
  files: process.env.FILESYSTEM_DEFAULT === 'local' ? 'storage/framework/sessions' : process.env.SESSION_FILES || 'storage/framework/sessions',
  connection: process.env.SESSION_CONNECTION,
  table: process.env.SESSION_TABLE || 'sessions',
  store: process.env.SESSION_STORE,
  cookie: process.env.SESSION_COOKIE || appName.replace(/[^a-z0-9-]+/gi, '-') + '-session',
