...
export function RegisterRequest(req, res, next) {
  ...
  // password
  if (!req.body.password || typeof req.body.password !== 'string' || req.body.password.length < 8 || req.body.password !== req.body.passwordConfirmation) {
    errors.push({ field: 'password', message: 'Password is required, must be at least 8 characters, and match the confirmation' });
  }
