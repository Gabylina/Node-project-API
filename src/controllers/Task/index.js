...
const normalizeStatus = s => { const v = String(s??'').toLowerCase().replace(/_/g,'-'); return ['pending','in-progress','completed'].includes(v) ? v : 'pending'; };
...
export async function changeStatus(req, res, next) {
  try {
    ...
    const newStatus = normalizeStatus(status);
    if (!['pending','in-progress','completed'].includes(newStatus)) {
      return res.status(400).json({ message: 'Estado de tarea inválido. Valores permitidos: pending, in-progress, completed.' });
    }
